...
/**
* SECTION:element-misbirpack
*
* Pack GRAY16 to MISB IR packed video.
*
* <refsect2>
* <title>Example launch line</title>
* |[
* gst-launch videotestsrc ! misbirpack ! misbirpack ! ffmpegcolorspace ! autovideosink
* ]|
* </refsect2>
*/
...
/* GstMisbIrPack signals and args */
enum
{
  /* FILL ME */
  LAST_SIGNAL
...
static GstStaticPadTemplate gst_misb_ir_pack_sink_template =
...
static GstStaticPadTemplate gst_misb_ir_pack_src_template =
...
static void gst_misb_ir_pack_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec);
static void gst_misb_ir_pack_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec);
static void gst_misb_ir_pack_dispose (GObject * object);

/* GstBaseTransform vmethod declarations */
static GstCaps *gst_misb_ir_pack_transform_caps (GstBaseTransform * trans,
    GstPadDirection direction, GstCaps * caps, GstCaps * filter_caps);

/* GstVideoFilter vmethod declarations */
static gboolean gst_misb_ir_pack_set_info (GstVideoFilter * filter,
    GstCaps * incaps, GstVideoInfo * in_info, GstCaps * outcaps,
    GstVideoInfo * out_info);
static GstFlowReturn gst_misb_ir_pack_transform_frame (GstVideoFilter * filter,
    GstVideoFrame * in_frame, GstVideoFrame * out_frame);

/* GstMisbIrPack method declarations */
static void gst_misb_ir_pack_reset (GstMisbIrPack * filter);

/* setup debug */
GST_DEBUG_CATEGORY_STATIC (misb_ir_pack_debug);
#define GST_CAT_DEFAULT misb_ir_pack_debug

G_DEFINE_TYPE (GstMisbIrPack, gst_misb_ir_pack, GST_TYPE_VIDEO_FILTER);
...
/**
 * gst_misb_ir_pack_dispose:
 * @object: #GObject.
 *
 */
static void
gst_misb_ir_pack_dispose (GObject * object)
{
  GstMisbIrPack *misb_ir_pack = GST_MISB_IR_PACK (object);

  GST_DEBUG ("dispose");

  gst_misb_ir_pack_reset (misb_ir_pack);

  /* chain up to the parent class */
  G_OBJECT_CLASS (gst_misb_ir_pack_parent_class)->dispose (object);
}

/**
 * gst_misb_ir_pack_class_init:
 * @object: #GstMisbIrPackClass.
 *
 */
static void
gst_misb_ir_pack_class_init (GstMisbIrPackClass * klass)
{
  ...
  gobject_class->dispose = GST_DEBUG_FUNCPTR (gst_misb_ir_pack_dispose);
  gobject_class->set_property =
      GST_DEBUG_FUNCPTR (gst_misb_ir_pack_set_property);
  gobject_class->get_property =
      GST_DEBUG_FUNCPTR (gst_misb_ir_pack_get_property);

  /* Install GObject properties */
  g_object_class_install_property (G_OBJECT_CLASS (klass),
      PROP_OFFSET, g_param_spec_int ("offset",
          "Offset value",
          "Offset value to apply during packing", 0, 1023,
          DEFAULT_PROP_OFFSET, G_PARAM_STATIC_STRINGS | G_PARAM_READWRITE));

  gst_element_class_add_pad_template (gstelement_class,
      gst_static_pad_template_get (&gst_misb_ir_pack_sink_template));
  gst_element_class_add_pad_template (gstelement_class,
...
GstCaps *
gst_misb_ir_pack_transform_caps (GstBaseTransform * trans,
    GstPadDirection direction, GstCaps * caps, GstCaps * filter_caps)
{
  GstMisbIrPack *filt = GST_MISB_IR_PACK (trans);
...
{
  GstMisbIrPack *filt = GST_MISB_IR_PACK (filter);
  gboolean res = TRUE;
...
gst_misb_ir_pack_transform_frame (GstVideoFilter * filter,
    GstVideoFrame * in_frame, GstVideoFrame * out_frame)
{
  GstMisbIrPack *filt = GST_MISB_IR_PACK (filter);
  GTimer *timer = NULL;
